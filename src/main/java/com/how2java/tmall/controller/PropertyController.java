...
public class PropertyController {
    ...
    /**
     *  1、在listProperty.jsp页面提交数据时，除了提交属性名称，还会提交cid
     *  2、在PropertyController通过参数property接受注入
     *  3、通过PropertyService保存到数据库
     *  4、客户端跳转至admin_property_list，并带上参数cid
     */
    ...
    public String delete(int id){
        ...
        return "redirect:admin property list?cid=" + p.getCid();
    }
    /**
     ...
     *  4、服务端跳转至admin/editProperty.jsp
     ...
     * 3、客户端跳转到admin_property_list，并带上cid
     * */
    ...
    /**
     *  查询地址admin_property_list映射的是PropertyController的list方法
     *  1、获取分类cid、和分页对象page
     *  2、通过PageHelper设置分页参数
     *  3、基于cid，获取当前分类下的属性集合
     *  4、通过PageInfo获取属性总数
     *  5、把总数设置给分页page对象
     *  6、拼接字符串"&cid=" + c.getId()，设置给page对象的Param值，因为属性分页都是基于当前分类下的分页，所以分页的时候要传递这个cid
     *  7、把属性集合设置到request的“ps”属性上
     *  8、把分类对象设置到request的“c”属性上
     *  9、把分页对象设置到request的“page”对象上
     *  10、服务端跳转到admin/listProperty.jsp页面
     *  11、在listProperty.jsp页面上使用c:forEach遍历ps集合并显示
     */
